...
Note, you should be using the BASH shell for this assignment. The `ssm-user` account uses the Bourne shell (`sh`) by default, so you may need to switch to the BASH shell by typing the command: `/bin/bash`. You can also configure AWS Session Manager to use the BASH shell by default by going to the AWS Systems Manager web console, selecting Session Manager, and selecting the Preferences menu tab. Here you can specify a Linux shell profile with commands that will automatically enable BASH and change to the user's home directory:
