...
The stack template should create the following logical resources:
  ...
  * A VPC called: *engineering-vpc*
    ** The VPC CIDR block should be: 10.0.0.0/18
    ** The VPC should have one subnet called: *public-subnet*
  ...
  * Two EC2 instances with logical resource names of *web1* and *web2* with associated *Name* tags.
    ** Both instances are located in the public-subnet.
  ...
  * A security group with a logical name and group name of *webservers-sg* in the VPC.
  ...
  * An application load balancer named *engineering-lb* and target group named *engineering-webservers*
