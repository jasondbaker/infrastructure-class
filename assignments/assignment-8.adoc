...
In this assignment you will revisit the Wordpress application again. This time we will try to automate most of the environment configuration using Cloudformation and Ansible. You will need to launch two EC2 instances and a MySQL RDS database for this assignment. We will configure the first instance as a management server with the Ansible tool installed and the second instance will become the Wordpress webserver. We will use the RDS database as the Wordpress database server.
...
The CloudFormation template is missing a couple resources that you need to add:
    ...
    * A `db.t2.micro` RDS database instance (not a cluster) running a MySQL 5.6.41 database called `wordpress` located in a private VPC subnet. Use the logical name `wordpressdb` for the CloudFormation RDS resource.
    ...
    * A stack output called `wordpressDbEndpoint` which shows the MySQL instance endpoint address.
...
You will create an Ansible playbook to configure the web1 instance as a Wordpress webserver. Before beginning the playbook, take a look at the facts that Ansible can collect from a remote host:
...
Here are the tasks you should configure in the first play:
    ...
    * Install the `httpd`, `php`, and `php-mysqlnd` software packages
    * Start the Apache web service and configure it to automatically start on system boot.

[WARNING]
====
While Ansible can run shell commands on servers using the `shell` or `command` modules, you will not receive credit for any configuration work completed in this assignment using shell commands. In general we try to avoid using shell commands in playbooks as much as possible because shell commands tend to be less maintainable. Playbooks are not shell scripts.
====
...
Review the Jinja template on the `mgmt1` server and notice how it's using the following variables: database_name, database_username, database_password, database_endpoint. The values of these variables must be set in the Ansible playbook before the template is processed.
...
Once you have written the plays in the playbook, execute the playbook to configure the `web1` host. If you encounter any error messages, review the playbook script and correct any mistakes. Configuration tools like Ansible are designed to be idempotent, meaning that you should be able to run the playbook multiple times. If for some reason the `web1` server is hopelessly misconfigured and you want to start over, you can always delete your CloudFormation stack and create a new one.

You should confirm that all the configuration tasks have been applied to the `web1` server and that the wordpress application is working before submitting your work.
