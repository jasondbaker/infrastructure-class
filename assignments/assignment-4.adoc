...
When you create a Git repository and start working with files, the main branch of the repository is checked out by default. Go ahead and create a file in the directory called `README.md` and add the following contents:

  *Assignment 4 Files*

Commit the file to the repository.

You just committed the file to the main branch of the repository. We generally try to avoid working directly on the main branch in our professional life. Go ahead and create a new branch in the Git repository called `develop` and check it out. This branch will have the README.md file in it because you just branched from the most recent commit in main.
...
* Merge your `develop` branch back into the `main` branch on your local Git repository.
* Push your local main branch up to the GitHub repository.
...
Create a new `develop` Git branch and check it out. Now add the `access2.log` file to the repository and commit your changes to the develop branch. Merge the `develop` branch into the `main` branch. Push the main branch back up to GitHub. Verify that the new `access2.log` file exists on
