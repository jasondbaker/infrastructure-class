...
e) Finally, the script should add the following two lines to the `wp-config.php` configuration file, using the DSN name of your load balancer instead of `(load balancer DNS name)`.
...
Create a small batch script called `getdata.sh` which performs the following:
  ...
  * Copies the output of the following command into a text file in the current
  ...
  aws autoscaling describe-scaling-activities --auto-scaling-group-name wordpress-web-group --region us-west-2 --output json
