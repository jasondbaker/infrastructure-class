...
First we need to install an installer repository:

  sudo yum install git -y
  git clone https://github.com/aws/aws-elastic-beanstalk-cli-setup.git
  
Next, we need to install some development tools and execute the ebcli installer. It will install Python into a virtual environment:
  ...
  sudo echo 'export PATH="/root/.ebcli-virtual-env/executables:$PATH"' >> ~/.bash_profile && source ~/.bash_profile
  sudo echo 'export PATH=/root/.pyenv/versions/3.*/bin:$PATH' >> /root/.bash_profile && source /root/.bash_profile  
  sudo python aws-elastic-beanstalk-cli-setup/scripts/ebcli_installer.py
