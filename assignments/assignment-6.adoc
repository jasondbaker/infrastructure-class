...
script should perform the following tasks in order:

1) Download the latest Wordpress software using this command:
...
2) Now that the script has downloaded the latest version of the Wordpress software, it should unpack the software archive:

  tar xf wp.tar.gz -C /var/www/html/

3) Remove the archive package (`wp.tar.gz`) from the /var/www/html directory.

4) The contents of the software archive package were installed in a new directory
called `wordpress` within the `/var/www/html/` directory. You need to configure the Wordpress software before
accessing it on the web server. Wordpress, like many software applications,
is configured using a text file containing settings and properties. Wordpress
comes with a sample configuration file. Your script should copy the sample configuration file to another file which you will use in production. In the `wordpress` directory, copy the `wp-config-sample.php` file to a file named `wp-config.php`.

5) Your script needs to set the MySQL database access information in the configuration file by modifying values in six places.
...
e) Finally, add the following two lines to the configuration file. You will need to enter the DNS name for your load balancer.
...
.Example
----
define('WP_HOME', 'http://wordpressLB-2127306763.us-west-2.elb.amazonaws.com');
define('WP_SITEURL', 'http://wordpressLB-2127306763.us-west-2.elb.amazonaws.com');
----
