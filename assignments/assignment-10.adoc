...
The project pipeline should include the following named stages (highlighted in bold):
  ...
  * **Deploy**
    ...
    - You can use the AWS CLI to copy files from Jenkins to the S3 bucket. Don't worry about access credentials because the Jenkins server has a proper role attached which allows it to access the S3 bucket.
...
Copy the Jenkinsfile you created for this assignment into this new repository and commit the code.
