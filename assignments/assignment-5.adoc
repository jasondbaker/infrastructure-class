...
Use the `curl` command with the `-vs` flag (verbose, silent) to create a file called `backend1-message.txt` containing
the data returned by accessing the following URL. You will need to redirect the output
