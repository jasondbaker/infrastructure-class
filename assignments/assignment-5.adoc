...
instance. The private ssh key is the key that you use to create an ssh connection to your EC2 instances.

You should still be connected to `webserver1` via your terminal program. Open
up the private ssh key on your local workstation in a text editing program and
...
We generally try to avoid working directly on the master branch in the real world. Go ahead and create a new branch called `develop` and check it out.
