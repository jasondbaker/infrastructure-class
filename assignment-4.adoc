...
= DevOps & Cloud Infrastructure: Assignment 4
...
can only associate an instance with a role when it's being launched. The role cannot
...
in the `/user/share/nginx` directory. This is the parent directory of your
...
instance when you launched it earlier. The role assigned full access
...
.Example
----
$ sudo aws s3 cp s3://jbaker-infrastructure-4/index.html html/index.html
----
...
command is a really basic text-based web browser. The curl browser accessed
...
constructing appropriate URLs. For example, if you want to retrieve the
...
instance meta-data is critically important when programmatically working with
EC2 instances.

=== Create a script

We're going to take our new found knowledge of instance meta-data and put it
to work. Start by creating a directory in `/user/share/nginx` (your current
...
Once your script is working properly, change back to the `/user/share/nginx`
directory and add the `scripts` directory and your new files to the Git
staging area. Confirm that your new script and its output file are in the
Git staging area. Commit the files to your Git repository.

In the previous assignment, we created a new repository on GitHub and then
...
need to recall the specific commands.
  ...
  * Create a new repository on GiHub called `infrastructure-assignment4`.
