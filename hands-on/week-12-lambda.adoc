...
Let's launch an EC2 instance containing some of the resources we will use to work on this project. Launch a public Amazon Linux 2 EC2 instance using the following AMI:
...
We will use the special `aws cloudformation package` command to create the Lambda package and CloudFormation SAM template. Run `aws config` and setup AWS API credentials on your EC2 instance. The credentials should be associated with an account that has at least `PowerUser` role access. 
