...
The public subnets are located in one availability zone and the private subnets are located in another. The exact AZs don't make a difference, as long as the AZs are different.

Create an Internet Gateway for the VPC.

Create a NAT gateway in the `ecotech-public1` subnet. Configure the routing tables for the public subnets and private subnets. Do you remember which subnets route through the Internet Gateway and which subnets route through the NAT gateway?
...
* Perform a health check on port 80 to the URL endpoint: `/input.php`
