...
Gatling will take a little while to start as the JVM loads up. Once it is running it will present you with a set of simulations it can run. Select the `webapp.RecordedSimulation`. You don't need to add a simulation description.

Once Gatling starts running you will start to see statistics scroll down the screen. What's going on here?

Gatling is generating thousands of web requests simulating hundreds of users accessing your web API application. The simulation is using two different types of users separated into Group1 and Group2. Each group is performing a slightly different set of requests.
...
Next, go to your Kibana instance and take a look at the data in the _Discovery_ menu. You can use your mouse to select a time interval in the chart and zoom in on the data. Try to answer the following questions:
  ...
  * How many times did uses requests information on Alicia?
