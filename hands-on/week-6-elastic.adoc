...
Gatling will take a little while to start as the JVM loads up. Once it is running it will present you with a set of simulations it can run. Select the `webapp.RecordedSimulation`.

Gatling will generate thousands of web requests simulating hundreds of users accessing the web API application. The _watiting_ statistic shows the number of users waiting to access the API. The _active_ statistic describes the number of active users currently making requests. The increasing bar graph at the bottom shows how much of the simulation has been completed.
